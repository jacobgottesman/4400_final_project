...
def get_device(device='auto'):
    ...
    Args:
        device: Requested device ('auto', 'cpu', 'cuda', 'mps', or specific like 'cuda:0')
    ...
    # For 'auto', try in order: MPS, CUDA, then fall back to CPU
...
class RunningRouteDataset(Dataset):
    ...
    def _get_bounds_hash(self, bounds):
        """Get a hash of the bounds for cache lookup"""
        # Convert bounds to string for hashing
        bounds_str = f"{bounds[0]:.5f}_{bounds[1]:.5f}_{bounds[2]:.5f}_{bounds[3]:.5f}"
    ...
    @staticmethod
    def convert_subgraph_to_pytorch_geometric(self, subgraph, center_node):
        """Convert NetworkX subgraph to PyTorch Geometric format"""
        # This method is used by the generate_route method
        # Include implementation here if needed
        # For now, return a placeholder implementation
        return create_empty_graph_data()
...
class GraphAwareRouteGenerator(nn.Module):
    ...
    def __init__(self, graph_feature_dim=256, condition_dim=64, hidden_dim=256, 
        ...
        # To store the device used by the model
    ...
    def to(self, device):
        """
        Override the to() method to keep track of the device
    ...
    def generate_route(self, graph, start_lat, start_lon, end_lat, end_lon, 
                       distance, max_length=500, device=None):
        """
        ...
        Args:
            ...
            device: Device to place tensors on (uses model's device if None)
...
class RouteModelTrainer:
    ...
    def __init__(self, model, train_loader, val_loader=None, 
                 lr=0.001, weight_decay=1e-5, device='auto', 
    ...
    def train(self, num_epochs, checkpoint_dir=None, initial_batch_size=None, 
        ...
        for epoch in range(num_epochs):
            ...
            # Generate a route comparison visualization if method is implemented
            if hasattr(self.model, 'visualize_route_comparison'):
                viz_path = os.path.join(checkpoint_dir, f'route_comparison_{epoch}.png') if checkpoint_dir else None
                try:
...
class RouteEvaluator:
    ...
    def __init__(self, model, test_dataset, device='auto', disable_graph=False):
        self.device = get_device(device)
        self.model = model.to(self.device)
...
def main():
    ...
    if args.eval_only and args.model_path:
        try:
            ...
            # Run evaluation
            evaluator = RouteEvaluator(model, test_dataset, device=device, 
    ...
    trainer = RouteModelTrainer(
        ...
        device=device,  # Using the properly handled device
